...
- **The reader draws its own chrome — it does NOT use the system nav bar.** `.toolbar(.hidden, for: .navigationBar)` proved unreliable at actually hiding a bar whose items were declared in the parent (`WorkDetailView` / `SavedWorkReader`), so the reader hides the system bar entirely and renders `floatingChrome` as a top `safeAreaInset`: a custom back button (`@Environment(\.dismiss)`) + a pill of actions + the chapter indicator. Parents pass their action buttons (follow/kudos/export/save) into `ReaderView` via a trailing `@ViewBuilder actions:` closure; the reader appends its own chapters + typography menus. Because the chrome is just a `if !chromeHidden { … }` view, hiding it on scroll is guaranteed (no toolbar-visibility quirks). Everything is painted with the reader's `background` (no `.ultraThinMaterial`/system gray), and the reader never uses `.preferredColorScheme` (it propagates to the whole window and flips the app light↔dark when you leave a light reader).
