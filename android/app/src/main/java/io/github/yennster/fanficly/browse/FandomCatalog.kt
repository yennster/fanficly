...
/**
 * Curated "popular" tags for the Popular tab — the offline fallback when no live
 * snapshot is available. AO3 has no popular/trending API, so these are
 * hand-picked canonical tag names (kept in AO3's exact canonical form so the tag
 * search resolves). Port of the iOS `PopularTags`.
 */
object PopularTags {
    val fandoms: List<String> = listOf(
...
/** 10 AO3 media categories + curated seed fandom lists. Port of iOS `FandomCatalog`. */
object FandomCatalog {

    /**
     * The icon for a fandom, matching the Browse category it belongs to. Exact
     * catalog match first, then AO3's canonical-name type suffixes. Port of the
     * iOS `FandomCatalog.symbol(for:)` heuristic.
     */
    fun iconFor(fandom: String): FandomIcon {
        symbolByFandom[fandom]?.let { return it }
        val lower = fandom.lowercase()
        fun has(vararg subs: String) = subs.any { lower.contains(it) }
        if (has("(anime", "manga", "アニメ", "| my hero", "no kyojin")) return FandomIcon.ANIME
        if (has("(video game", "(visual novel", "(vrchat")) return FandomIcon.GAME
        if (has("(podcast", "(radio", "(audio")) return FandomIcon.AUDIO
        if (has("musical", "broadway", "- miranda", "lloyd webber")) return FandomIcon.THEATER
        if (has("cinematic universe", "movies)", "(movie", "(film")) return FandomIcon.FILM
        if (has("(cartoon", "(web series", "(webcomic", "(web comic")) return FandomIcon.CARTOON
        if (has("(tv", " tv ", "us tv)", "uk tv)")) return FandomIcon.TV
        if (has("rpf")) return FandomIcon.PERSON
        if (has("(band", "(musician", "k-pop", "kpop", "방탄소년단")) return FandomIcon.MUSIC
        return FandomIcon.BOOK
    }

    private val symbolByFandom: Map<String, FandomIcon> by lazy {
        buildMap { all.forEach { c -> c.fandoms.forEach { put(it.canonicalName, c.icon) } } }
    }

    val all: List<FandomCategory> = listOf(
        FandomCategory("anime-manga", "Anime & Manga", FandomIcon.ANIME, listOf(
        ...
        FandomCategory("books", "Books & Literature", FandomIcon.BOOK, listOf(
        ...
        FandomCategory("cartoons-comics", "Cartoons & Comics & Graphic Novels", FandomIcon.CARTOON, listOf(
            BrowseFandom("Marvel"),
            BrowseFandom("DCU"),
        ...
        FandomCategory("celebrities-rpf", "Celebrities & Real People", FandomIcon.PERSON, listOf(
        ...
        FandomCategory("movies", "Movies", FandomIcon.FILM, listOf(
        ...
        FandomCategory("music-bands", "Music & Bands", FandomIcon.MUSIC, listOf(
        ...
        FandomCategory("other-media", "Other Media", FandomIcon.AUDIO, listOf(
        ...
        FandomCategory("theater", "Theater", FandomIcon.THEATER, listOf(
        ...
        FandomCategory("tv-shows", "TV Shows", FandomIcon.TV, listOf(
        ...
        FandomCategory("video-games", "Video Games", FandomIcon.GAME, listOf(
